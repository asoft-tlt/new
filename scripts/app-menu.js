...
window.App = {
  ...
  init: function() {
    ...
    this.circlePlayer = $('#audioPleer');
    /*new CirclePlayer('.audio-player', {}, {
...
*/
    ...
    $('.cp-play').on('click', this.playAudio.bind(this));
    $('.cp-pause').on('click', this.pauseAudio.bind(this));
    // Hide articles
    ...
    switch (page.type) {
      ...
      case 'page':
        ...
        if (page.sounds && page.sounds.length) {
          ...
          App.showAndPlayAudio(page.sounds, {
            id: page.id
          });
          //}, 0);
        }
  ...
  showAndPlayAudio: function(audio, params) {
    ...
    if (files.length > 0) {
      ...
      $('.jp-playlist ul').html(htmlNames);
      $('.jp-playlist ul').css('transform', 'translate3d(0px, 0px, 0px)');

      this.playNewAudio(files);
      $('.jp-playlist-current').on('click', this.setAudio.bind(this));
      //this.isPlaying = true;
    } else {
  ...
  playNewAudio: function(src) {
    ...
    if (src && src.length > 0) {
      this.circlePlayer.attr('src', src);
      if (this.circlePlayerPromise !== undefined) {
            this.circlePlayerPromise.then(_ => {
  ...
  setAudio: function(item) {

    var aT = $(item.currentTarget).find('.jp-playlist-item');

    this.playNewAudio($(aT).attr('src'));
  ...
  hideAndStopAudio: function() {
    if (this.circlePlayerPromise !== undefined) {
      this.circlePlayerPromise.then(_ => {
        ...
        this.circlePlayer.get(0).pause();
        this.circlePlayer.attr('src', '');
  ...
  playAudio: function() {
    if (this.circlePlayer.get(0).paused) {
      this.circlePlayer.get(0).play();
      $('.cp-play').hide();
      $('.cp-pause').show();
    }
  },
  pauseAudio: function() {
    //  console.log(this.circlePlayerPromise);
    //    console.log(this.circlePlayer.get(0).duration);
    if (this.circlePlayer.get(0).paused !== true) {
      //        console.log('pause');
